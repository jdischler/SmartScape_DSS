...
public class CropRotation
{
	...
	private void processLayers()
	{
		...
		int inCORN = 1, inGRAINS = 2, inVEGGIES = 3, inTREECROP = 4, inGRASS = 6, inWOODS = 7, inWETLAND = 8,
			...
			inALFALFA = 17, inCORN_GRAIN = 18, inSOY_GRAIN = 19;
			
		int CONTINUOUS_CORN = 100, CONTINUOUS_SOY = 200, CORN_SOY = 300, OTHER_GRAIN = 500, VEGGIES = 600, TREE_CROPS = 700,
			...
			for (int y = 0; y < height; y++) {
				...
				else
				{
					...
					if (result != NO_DATA && result != NO_CONDITION) {
						// Data value 'constants' multiplied by 100 when defined to avoid colliding with
	...
	private int anyTwoYearRotation(int type1, int type2, int one, int two, int three, int rotation, int current) {
		if ((one == type1 || one == type2) &&
				(two == type1 || two == type2) &&
				(three == type1 || three == type2) &&
				(one != two || two != three)) {
			return rotation;
		}
		return current;
	...
	private int matchThree(int type1, int type2, int type3, int one, int two, int three, int output, int current) {
		if ((one == type1 || one == type2 || one == type3) &&
				(two == type1 || two == type2 || two == type3) &&
				(three == type1 || three == type2 || three == type3) &&
